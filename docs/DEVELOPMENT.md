...
│   │                                       # (the tauri implementation lives in a separate repository, see README)
...
### Sandbox on Ubuntu 24.04+ <a id="linux-sandbox"></a>

Ubuntu 24.04 and later [restrict unprivileged user namespaces via AppArmor](https://chromium.googlesource.com/chromium/src/+/main/docs/security/apparmor-userns-restrictions.md). Electron's Chromium sandbox relies on user namespaces, so running `pnpm start` may fail with a sandbox error.
...
> **Note:** The packaged `.deb` already ships an AppArmor profile automatically (via electron-builder), so this is only needed for development.

To remove the profile again:

```sh
sudo apparmor_parser --remove /etc/apparmor.d/deltachat-desktop-dev
sudo rm /etc/apparmor.d/deltachat-desktop-dev
```
