...
export async function updateCoreStrings() {
  ...
  const strings: StockStringsSomeOmited = {
    ...
    [C.DC_STR_INCOMING_AUDIO_CALL]: tx('incoming_audio_call'),
    [C.DC_STR_OUTGOING_AUDIO_CALL]: tx('outgoing_audio_call'),
    [C.DC_STR_INCOMING_VIDEO_CALL]: tx('incoming_video_call'),
    [C.DC_STR_OUTGOING_VIDEO_CALL]: tx('outgoing_video_call'),
