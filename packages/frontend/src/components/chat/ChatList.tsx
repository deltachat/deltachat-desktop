...
export function useLogicVirtualChatList(chatListIds: number[]) {
  ...
  const loadingChats_ = useRef(new Set<T.BasicChat['id']>())
  const loadingChats = loadingChats_.current
...
function useMessageResults(
  ...
  const loadingMessages_ = useRef(new Set<T.MessageSearchResult['id']>())
  const loadingMessages = loadingMessages_.current
