...
export function startOutgoingVideoCall(accountId: number, chatId: number) {
  ...
  const offerP = new Promise<string>(r => {
    port.once('message', e => {
  ...
  ;(async () => {
    ...
    port.postMessage({ type: 'answer', answer: answer satisfies string })
  })()
...
function listenForAnswerFromCallee(
  ...
  const callAcceptedListener = (
    ...
    log.info('got OutgoingCallAccepted event', accountId, msg_id)
