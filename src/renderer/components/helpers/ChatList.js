...
export const useLazyChatListItems = chatListIds => {
  ...
  const [fetchChatsInView] = useDebouncedCallback(async () => {
    ...
    const chatIds = chatIdsInView(10)
  ...
  useEffect(() => {
    ...
    ipcBackend.on('DC_EVENT_MSGS_CHANGED', refetchChatIfInViewUnsetOtherwise)
    ipcBackend.on('DC_EVENT_INCOMING_MSG', refetchChatIfInViewUnsetOtherwise)
    ipcBackend.on('DC_EVENT_MSG_DELIVERED', refetchChatIfInViewUnsetOtherwise) 
    return () => {
      ipcBackend.removeListener('DC_EVENT_MSGS_CHANGED', refetchChatIfInViewUnsetOtherwise)
      ipcBackend.removeListener('DC_EVENT_INCOMING_MSG', refetchChatIfInViewUnsetOtherwise)
      ipcBackend.removeListener('DC_EVENT_MSG_DELIVERED', refetchChatIfInViewUnsetOtherwise) 
    }
  }, [chatListIds])
