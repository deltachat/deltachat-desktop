...
ipcRenderer.on('DC_EVENT_MSGS_CHANGED', (evt, payload) => {
  const { selectedChat, mapSettings } = locationStore.getState()
...
ipcRenderer.on('DC_EVENT_INCOMING_MSG', (evt, payload) => {
  const { selectedChat, mapSettings } = locationStore.getState()
  const { chatId } = payload
  if (selectedChat && chatId === selectedChat.id) {
    getLocations(chatId, mapSettings)
...
ipcRenderer.on('DD_EVENT_LOCATIONS_UPDATED', (evt, payload) => {
  ...
  locationStore.setState({ ...locationStore.getState(), locations: locations })
})

locationStore.reducers.push((action, state) => {
  if (action.type === 'DC_GET_LOCATIONS') {
    const { timestampFrom, timestampTo } = action.payload
    state = Object.assign({}, state, { mapSettings: { timestampFrom: timestampFrom, timestampTo: timestampTo } })
    return state
  }
})
