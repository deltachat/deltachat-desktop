...
import { BackendRemote, ApiService, Type } from '../_apiService'
...
class ChatStore extends Store<ChatStoreState> {
  ...
  effect = {
    ...
    selectChat: async (chatId: number) => {
      ...
      const chat = await ApiService.getFullChatById(accountId, chatId)
      if (chat.id === null) {
        log.debug(
      ...
      ApiService.marknoticedChat(accountId, chatId)
