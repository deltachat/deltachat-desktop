...
class SettingsStore extends Store<SettingsStoreState | null> {
  ...
  effect = {
    load: async() => {
      const settings = await DeltaBackend.call('settings.getConfigFor', settingsKeys) as SettingsStoreState['settings']
      const selfContact = await DeltaBackend.call('contacts.getContact', C.DC_CONTACT_ID_SELF)
